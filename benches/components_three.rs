...
impl<'a> Components<'a> {
    ...
    pub fn as_path(&self) -> &'a Path {
        ...
        if self.front == State::Body {
            while len != 0 {
                ...
                let path = unsafe { slice::from_raw_parts(self.path.add(front), len) };
                let (extra, comp) = match path.iter().position(|b| self.is_sep_byte(*b)) {
                    None => (0, path),
                    Some(i) => (1, &path[..i]),
                };
        ...
        if self.back == State::Body {
            while len > self.len_before_body() {
        ...
        unsafe { from_u8_slice(slice::from_raw_parts(self.path.add(front), len)) }
    }
    ...
    fn parse_next_component(&self) -> (usize, Option<Component<'a>>) {
        debug_assert!(self.front == State::Body);
        let path = unsafe { slice::from_raw_parts(self.path, self.len) };
        let (extra, comp) = match path.iter().position(|b| self.is_sep_byte(*b)) {
            None => (0, path),
            Some(i) => (1, &path[..i]),
        };
    ...
    fn parse_next_component_back(&self) -> (usize, Option<Component<'a>>) {
        ...
        let path = unsafe { slice::from_raw_parts(self.path, self.len) };
        let start = self.len_before_body();
        let (extra, comp) = match path[start..].iter().rposition(|b| self.is_sep_byte(*b)) {
            None => (0, &path[start..]),
    ...
    fn trim_right(&mut self) {
        while self.len > self.len_before_body() {
            let (size, comp) = self.parse_next_component_back();
            if comp.is_some() {
                return;
            } else {
            ...
            match self.front {
                ...
                State::StartDir => {
                    ...
                    if self.has_physical_root {
                        ...
                        self.path = unsafe { self.path.add(1) };
                        self.len -= 1;
...
impl<'a> DoubleEndedIterator for Components<'a> {
    ...
    fn next_back(&mut self) -> Option<Component<'a>> {
        while !self.finished() {
            match self.back {
                State::Body if self.len > self.len_before_body() => {
                    let (size, comp) = self.parse_next_component_back();
                    self.len -= size;
                    if comp.is_some() {
                        return comp;
...
fn compare_components(mut left: Components<'_>, mut right: Components<'_>) -> cmp::Ordering {
    ...
    // the middle of one
    let left_path = unsafe { slice::from_raw_parts(left.path, left.len) };
    let right_path = unsafe { slice::from_raw_parts(right.path, right.len) };
    if left.front == right.front {
        // possible future improvement: a [u8]::first_mismatch simd implementation
        let first_difference = match left_path.iter().zip(right_path).position(|(&a, &b)| a != b) {
            None if left_path.len() == right_path.len() => return cmp::Ordering::Equal,
            None => left_path.len().min(right_path.len()),
            Some(diff) => diff,
        };
            ...
            left_path[..first_difference].iter().rposition(|&b| left.is_sep_byte(b))
        {
            ...
            right.path = unsafe { right.path.add(mismatched_component_start) };
...
fn components(path: &Path) -> Components<'_> {
    ...
    Components {
        ...
        path: path.as_ptr(),
        len: path.len(),
        has_physical_root: has_physical_root(path),
...
fn bench_components_fast_two(c: &mut Criterion) {
    ...
    // c.bench_function("RawStateComponents Rewrite", |b| {
    //     b.iter(|| black_box(components_iter(black_box(path.as_ref()))))
    // });

    // c.bench_function("RawStateComponents Next Rewrite", |b| {
    //     b.iter(|| black_box(components_next_iter(black_box(path.as_ref()))))
    // });

    // c.bench_function("RawStateComponents Next Back Rewrite", |b| {
    //     b.iter(|| black_box(components_next_back_iter(black_box(path.as_ref()))))
    // });

    // c.bench_function("RawStateComponents Path Iter Rewrite", |b| {
    //     b.iter(|| black_box(path_iter(black_box(path.as_ref()))))
    // });

    c.bench_function("RawStateComponents As Path Iter Rewrite", |b| {
        b.iter(|| black_box(as_path_iter(black_box(path.as_ref()))))
    });

    c.bench_function("RawStateComponents Eq Comps Rewrite", |b| {
        b.iter(|| black_box(eq_comps(black_box(path.as_ref()), black_box(path.as_ref()))))
    });

    c.bench_function("RawStateComponents Uneq Comps Rewrite", |b| {
        b.iter(|| {
            black_box(eq_comps(
    ...
    c.bench_function("RawStateComponents Uneq 2 Comps Rewrite", |b| {
        b.iter(|| {
            black_box(eq_comps(
                black_box(path.as_ref()),
    ...
    // c.bench_function("RawStateComponents Compare Comps Rewrite", |b| {
    //     b.iter(|| {
    ...
    // c.bench_function("RawStateComponents Compare Uneq Comps Rewrite", |b| {
    //     b.iter(|| {
    //         black_box(compare_comps(
    //             black_box(path.as_ref()),
    //             black_box(path_b.as_ref()),
    ...
    // c.bench_function("RawStateComponents Compare Uneq 2 Comps Rewrite", |b| {
    //     b.iter(|| {
    //         black_box(compare_comps(
    //             black_box(path.as_ref()),
    //             black_box(path_c.as_ref()),
